...
with open('/tmp/udids.csv', 'rt') as f:
    reader = csv.reader(f)
    my_list = list(reader)
...
for user in MDMUser.get_users_and_command_queue(device_ids=udids,
                locking=LockingStrategy.FOR_UPDATE_OF_ANY,
                exclude_users=True
            ):
...
echo "exec(open('removeAppPortal.py').read())" | /usr/local/filewave/python/bin/python /usr/local/filewave/django/manage.pyc shell
